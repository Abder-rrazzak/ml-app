...
def train_with_mlflow(
    ...
    try:
        ...
        cv_scores = cross_val_score(
            classifier.model, X_train, y_train, cv=cv_strategy, scoring="accuracy"
        )
        ...
        logger.info(
            f"Validation croisée - Accuracy: {cv_metrics['cv_mean_accuracy']:.4f} "
        ...
        logger.info(
            f"Validation croisée: {cv_metrics['cv_mean_accuracy']:.4f} ±{cv_metrics['cv_std_accuracy']:.4f}"
        )
...
@click.option(
    "--cv-folds", default=5, type=int, help="Nombre de folds pour la validation croisée"
)
@click.option(
    "--experiment-name",
...
@click.option("--verbose", is_flag=True, help="Affichage détaillé des logs")
def train_advanced(
    model_path, random_state, test_size, cv_folds, experiment_name, verbose
):
    """
    ...
    try:
        ...
        click.echo(
            f"🔄 Validation croisée: {results['cv_accuracy']:.4f} ±{results['cv_std']:.4f}"
        )
