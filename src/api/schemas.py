...
class PredictionResponse(BaseModel):
    ...
    probabilities: Optional[List[List[float]]] = Field(
        None, description="Probabilités pour chaque classe"
    )

    # Nombre de prédictions effectuées
    count: int = Field(..., description="Nombre de prédictions dans cette réponse")

    # Identifiant de la requête (si fourni)
    request_id: Optional[str] = Field(
        None, description="Identifiant de la requête originale"
    )

    # Version du modèle utilisé
    model_version: str = Field(
        ..., description="Version du modèle utilisé pour la prédiction", example="1.0.0"
    )
...
class ErrorResponse(BaseModel):
    ...
    details: Optional[str] = Field(
        None, description="Détails supplémentaires sur l'erreur"
    )
